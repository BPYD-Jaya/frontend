...
export default function AddMasterProduct() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await Axios.post("https://backend.ptwpi.co.id/api/categories/create", formData, {
        headers: {
      ...
      <div className="flex-grow h-full ml-4 md:ml-80 pt-10 mr-4">
        <form onSubmit={handleFormSubmit}>
          <div className="grid md:grid-cols-4 gap-2 bg-white md:mr-6 mb-6 pt-6 pb-6 px-6 rounded-lg shadow-md">
            ...
            <div className="md:col-span-4 border border-gray-400 w-full rounded-lg  h-[150px] flex justify-center items-center" {...getRootProps()}>
              <input {...getInputProps()} />
              {isDragActive ? (
                ...
                <div className="text-center flex flex-col items-center">
                  ...
                  <p className="mt-2">{selectedFile ? `File: ${selectedFile.name}` : "Drag and drop file here or click to select file"}</p>
                </div>
              )}
            </div>
